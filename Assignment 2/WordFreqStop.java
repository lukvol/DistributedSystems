...
public class WordFreqStop extends Configured implements Tool {
	...
	public static class CountMapper extends Mapper<Object, Text, Text, IntWritable> {
		...
		public void setup(Context context) throws IOException {
			//we get access to the filesystem through the context
			//because we want to load our file through the filesystem
			FileSystem fs = FileSystem.get(context.getConfiguration());
			
			//get the locations as URI[] from our context
			URI[] locations = context.getCacheFiles();
			//we only saved one file in the cache, therefore 0 is the way to go
			//we convert the URI to a path
			Path path = new Path(locations[0].getPath());
			
			//we create a InputStreamReader by opening the path with our filesystem
			//and then pass the InputStreamReader to a BufferedReader
			InputStreamReader fileReader = new InputStreamReader(fs.open(path));
		...
		public void map(Object key, Text value, Context context)
			...
			while (tokenizer.hasMoreTokens()) {
				...
				//if we encounter a stopWord, we go to the next token
				if (stopWords.contains(keyForReduce)) {
					continue;
				}
	...
	public int run(String[] args) throws Exception {
		...
		final String stopLocation = args[0] +"stopwords/stop.txt";
		...
		job1.setJarByClass(WordFreqStop.class);
		...
		//cache stop.txt
		job1.addCacheFile(new Path(stopLocation).toUri());
		...
		//we pass our sorting-implementation as argument
	...
	public static void main(String[] args) throws Exception {
		ToolRunner.run(new Configuration(), new WordFreqStop(), args);
	}
}